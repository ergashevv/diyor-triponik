<template>
  <div class="bg-gray-50 min-h-screen">
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="showFilterDrawer" class="fixed inset-0 bg-black/50 z-[100] lg:hidden" @click="showFilterDrawer = false"></div>
      </Transition>
      ...
      >
        <div v-if="showFilterDrawer" class="fixed top-0 right-0 bottom-0 w-[85%] max-w-sm bg-white shadow-2xl z-[101] overflow-y-auto lg:hidden">
          <div class="sticky top-0 bg-white border-b border-gray-200 px-4 py-4 flex items-center justify-between z-10">
            ...
            <button @click="showFilterDrawer = false" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors">
          ...
          <div class="sticky bottom-0 bg-white border-t border-gray-200 px-4 py-3">
            <button @click="showFilterDrawer = false" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition-colors">
              Natijalarni ko'rish
            </button>
          </div>
        </div>
    ...
    >
      <div class="flex items-center justify-between mb-4 md:mb-6 gap-4 md:gap-8">
        ...
        <div class="flex items-center gap-2 md:gap-3 shrink-0">
          <button @click="decrementAdults" type="button" class="w-7 h-7 md:w-8 md:h-8 border border-gray-300 rounded-lg flex items-center justify-center hover:bg-gray-50" :disabled="passengers.adults <= 1">
          ...
          <button @click="incrementAdults" type="button" class="w-7 h-7 md:w-8 md:h-8 border border-blue-600 rounded-lg flex items-center justify-center hover:bg-blue-50 text-blue-600">
            <svg class="w-3.5 h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex items-center justify-between gap-4 md:gap-8">
        ...
        <div class="flex items-center gap-2 md:gap-3 shrink-0">
          <button @click="decrementChildren" type="button" class="w-7 h-7 md:w-8 md:h-8 border border-gray-300 rounded-lg flex items-center justify-center hover:bg-gray-50" :disabled="passengers.children <= 0">
            <svg class="w-3.5 h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
            </svg>
          </button>
          <span class="text-gray-900 text-sm md:text-base font-medium w-6 md:w-8 text-center">{{ passengers.children }}</span>
          <button @click="incrementChildren" type="button" class="w-7 h-7 md:w-8 md:h-8 border border-blue-600 rounded-lg flex items-center justify-center hover:bg-blue-50 text-blue-600">
            <svg class="w-3.5 h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
            </svg>
          </button>
...
const getStationSuggestions = (query) => {
  ...
  return cityNames.filter(station => station.toLowerCase().includes(query.toLowerCase()))
}
...
const handleFromFocus = () => {
  showFromSuggestions.value = true
  if (fromSuggestions.value.length === 0) fromSuggestions.value = getStationSuggestions('')
}

const handleToFocus = () => {
  showToSuggestions.value = true
  if (toSuggestions.value.length === 0) toSuggestions.value = getStationSuggestions('')
}

const handleFromBlur = () => { setTimeout(() => { showFromSuggestions.value = false; activeFromIndex.value = -1 }, 200) }
const handleToBlur = () => { setTimeout(() => { showToSuggestions.value = false; activeToIndex.value = -1 }, 200) }
const selectFromStation = (station) => { searchParams.value.from = station; showFromSuggestions.value = false; activeFromIndex.value = -1 }
const selectToStation = (station) => { searchParams.value.to = station; showToSuggestions.value = false; activeToIndex.value = -1 }
const swapStations = () => { const temp = searchParams.value.from; searchParams.value.from = searchParams.value.to; searchParams.value.to = temp }
...
const loadRecentSearches = () => {
  if (typeof window !== 'undefined') {
    ...
    if (saved) try { recentSearches.value = JSON.parse(saved).slice(0, 3) } catch (e) { recentSearches.value = [] }
  }
}

const saveSearchToHistory = () => {
  if (typeof window !== 'undefined' && searchParams.value.from && searchParams.value.to) {
    const newSearch = { from: searchParams.value.from, to: searchParams.value.to, departureDate: searchParams.value.departureDate, timestamp: Date.now() }
    let searches = recentSearches.value.filter(s => !(s.from === newSearch.from && s.to === newSearch.to))
    searches.unshift(newSearch)
    recentSearches.value = searches.slice(0, 3)
    localStorage.setItem('recentTrainSearches', JSON.stringify(recentSearches.value))
  }
}
  ...
  try {
    const data = await $fetch('/api/trains/search', {
      params: {
        ...
        date: searchParams.value.departureDate
      }
    })
    trains.value = data
...
const applyFilters = () => {
  ...
  filtered = filtered.filter(train => {
    const prices = Object.values(train.availableSeats)
      .filter(seat => seat.available !== 'Joy yo\'q' && seat.available !== 'Joy yoq')
      .map(seat => seat.price)
  ...
  if (filters.value.departureTime.length > 0) {
    filtered = filtered.filter(train => filters.value.departureTime.includes(train.timeSlot))
  }

  // 3. Poyezd turlari filtri
  if (filters.value.trainTypes.length > 0) {
    filtered = filtered.filter(train => filters.value.trainTypes.includes(train.type))
  }
  ...
  if (filters.value.wagonTypes.length > 0) {
    filtered = filtered.filter(train => {
      return filters.value.wagonTypes.some(wagonType => 
        train.availableSeats[wagonType] && 
        train.availableSeats[wagonType].available !== 'Joy yo\'q' && 
        train.availableSeats[wagonType].available !== 'Joy yoq'
      )
    })
  }
...
const handleSearch = () => {
  displayedSearch.value = { ...searchParams.value }
  saveSearchToHistory()
  router.push({ path: '/trains/search', query: { ...searchParams.value } })
  fetchTrains()
}

const handleClickOutside = (event) => {
  if (fromInputRef.value && !fromInputRef.value.contains(event.target)) showFromSuggestions.value = false
  if (toInputRef.value && !toInputRef.value.contains(event.target)) showToSuggestions.value = false
  if (passengersButtonRef.value && !passengersButtonRef.value.contains(event.target)) {
    if (passengersDropdownRef.value && !passengersDropdownRef.value.contains(event.target)) showPassengersDropdown.value = false
  }
}
...
onMounted(() => {
  ...
  window.addEventListener('scroll', () => { if (showPassengersDropdown.value) updatePassengersPosition() })
  window.addEventListener('resize', () => { if (showPassengersDropdown.value) updatePassengersPosition() })
})
...
.custom-calendar :deep([data-state="selected"]) { background-color: #2563EB !important; color: white !important; }
.custom-calendar :deep(button[data-today="true"]),
.custom-calendar :deep([data-state="today"]) { border: 2px solid #2563EB !important; font-weight: 600; }
</style>
