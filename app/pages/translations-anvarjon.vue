  ...
  <div class="p-4 min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400">Anvarjon Workspace</p>
      </div>
    </div>
...
async function saveTranslation() {
  ...
  try {
    if (isEditing.value) {
      await $fetch(`/api/i18n/translations/anvarjon/${encodeURIComponent(editingKey.value)}`, {
        method: 'PUT',
        body: newTranslation.value
      })
      toast.add({ title: 'Yangilandi', color: 'success' })
    } else {
      await $fetch('/api/i18n/translations-anvarjon', {
        method: 'POST',
        body: newTranslation.value
      })
...
async function deleteTranslation() {
  saving.value = true
  try {
    await $fetch(`/api/i18n/translations/anvarjon/${encodeURIComponent(deletingItem.value.key)}`, {
      method: 'DELETE'
    })
...
async function fetchTranslations() {
  ...
  try {
    ...
    const result = await $fetch(`/api/i18n/translations-anvarjon?${params}`)
    if (result.success) {
